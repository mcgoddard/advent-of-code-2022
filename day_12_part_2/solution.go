...
func parseGraph(scanner *bufio.Scanner) ([][]int, int, []int) {
	...
	for i, line := range lines {
		...
		for j, c := range line {
			if c == 'S' {
				endIndexes = append(endIndexes, j+(i*len(lines[0])))
				rows[i][j] = 0
			} else if c == 'E' {
	...
	return verticies, startIndex, endIndexes
}
...
func main() {
	...
	verticies, startIndex, endIndexes := parseGraph(scanner)
	distance, _ := dijkstra(verticies, startIndex)

	minSteps := intMax
	for _, endIndex := range endIndexes {
		if distance[endIndex] < minSteps {
			minSteps = distance[endIndex]
		}
	}
