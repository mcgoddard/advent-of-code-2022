...
func make_move(instruction Move, stacks [][]rune) [][]rune {
	for i := 0; i < instruction.count; i++ {
		character := stacks[instruction.from][0]
		stacks[instruction.from] = stacks[instruction.from][1:]
		stacks[instruction.to] = append([]rune{character}, stacks[instruction.to]...)
	}
	return stacks
}
...
func main() {
	...
	for _, instruction := range instructions {
		stacks = make_move(instruction, stacks)
	}
